...
const ADMIN_DOCS_DIR = path.join(process.cwd(), "data", "docs", "admin");
const PUBLIC_DOCS_DIR = path.join(process.cwd(), "data", "docs", "public");
const METADATA_FILE = path.join(process.cwd(), "data", "docs_metadata.json");
const ADMIN_METADATA_FILE = path.join(process.cwd(), "data", "docs_admin_metadata.json");

// Helper functions for metadata management
function loadMetadata(adminMode: boolean = false): Record<string, { originalName: string; uploadedAt: string }> {
  try {
    const metadataFile = adminMode ? ADMIN_METADATA_FILE : METADATA_FILE;
...
function saveMetadata(metadata: Record<string, { originalName: string; uploadedAt: string }>, adminMode: boolean = false) {
  try {
    const metadataFile = adminMode ? ADMIN_METADATA_FILE : METADATA_FILE;
...
async function loadDocs(adminMode: boolean = false): Promise<{ name: string; text: string }[]> {
  const docsDir = adminMode ? ADMIN_DOCS_DIR : PUBLIC_DOCS_DIR;
  
  if (!fs.existsSync(docsDir)) {
    console.warn('[loadDocs] Directory does not exist:', docsDir);
    return [];
  }
  
  const files = fs.readdirSync(docsDir);
  console.log(`[loadDocs] Found ${files.length} files in ${docsDir} (adminMode: ${adminMode})`);
  ...
  console.log(`[loadDocs] Total indexed: ${out.length} documents (adminMode: ${adminMode})`);
  return out;
}
...
router.post("/chat", async (req, res) => {
  const { message, adminMode } = req.body || {};
  if (!message) return res.status(400).json({ error: "message required" });

  const isAdminMode = adminMode === true || adminMode === 'true';
  const userRole = (req as any).user?.role;
  
  // Only admin/manager can use admin mode
  if (isAdminMode && userRole !== 'admin' && userRole !== 'manager') {
    return res.status(403).json({ error: "Admin mode is restricted to administrators and managers" });
  }

  console.log(`[/ai/chat] Question: "${message}" (adminMode: ${isAdminMode}, role: ${userRole})`);
  
  const docs = await loadDocs(isAdminMode);
...
router.post("/documents/upload", authorize('admin', 'manager'), upload.single("file"), (req, res) => {
  ...
  const { adminMode } = req.body || {};
  const isAdminMode = adminMode === true || adminMode === 'true';
  const docsDir = isAdminMode ? ADMIN_DOCS_DIR : PUBLIC_DOCS_DIR;
  ...
  const ext = path.extname(req.file.originalname).toLowerCase() || ".txt";
    ...
    const dest = path.join(docsDir, storedName);
    ...
    const metadata = loadMetadata(isAdminMode);
    metadata[storedName] = {
      originalName: req.file.originalname,
      uploadedAt: new Date().toISOString()
    };
    saveMetadata(metadata, isAdminMode);
  
    console.log(`[/ai/documents/upload] Uploaded: ${req.file.originalname} → ${storedName} (adminMode: ${isAdminMode})`);
...
router.get('/documents', (req, res) => {
  try {
    const adminMode = String(req.query.adminMode || '') === 'true';
    const userRole = (req as any).user?.role;
    
    // Only admin/manager can access admin documents
    if (adminMode && userRole !== 'admin' && userRole !== 'manager') {
      return res.status(403).json({ error: "Admin mode is restricted to administrators and managers" });
    }
    
    const docsDir = adminMode ? ADMIN_DOCS_DIR : PUBLIC_DOCS_DIR;
    if (!fs.existsSync(docsDir)) return res.json({ files: [] });
    
    const metadata = loadMetadata(adminMode);
    
    const files = fs.readdirSync(docsDir)
      .filter(f => /\.(md|txt|csv|pdf|docx?|xlsx?|png|jpe?g)$/i.test(f))
      .map((storedName) => {
        const filePath = path.join(docsDir, storedName);
...
router.delete('/documents/:name', authorize('admin', 'manager'), (req, res) => {
  try {
    ...
    if (metadata[fileName]) {
      ...
      saveMetadata(metadata, adminMode);
    }
    
    fs.unlinkSync(filePath);
    console.log(`[/ai/documents/:name] Deleted: ${fileName} (adminMode: ${adminMode})`);
