...
export default function Payroll() {
  // IMMEDIATE role check - block non-admins at component level
  const [role, setRole] = useState<string | null>(null);
  const [roleChecked, setRoleChecked] = useState(false);
  
  useEffect(() => {
    // Check role immediately on mount
    const currentRole = localStorage.getItem('role');
    const normalizedRole = currentRole ? String(currentRole).toLowerCase().trim() : null;
