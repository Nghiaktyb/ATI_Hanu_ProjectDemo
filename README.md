...
| ----------------------------------------------------------------------- | ------------------------ | ------------------------------------------------------------------------- |
| You want the fastest start with everything containerized                | **Docker ()** | Works the same on Windows/macOS/Linux, hides local dependency differences |
| You already have Node/MySQL installed or must connect to a corporate DB | **Manual setup (recommended)**         | Lets you point the backend to any database and tweak code live            |
