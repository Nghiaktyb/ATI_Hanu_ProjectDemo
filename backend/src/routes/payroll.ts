...
// POST /payroll/run - Only Admin/Manager can run payroll
router.post("/run", authorize('admin', 'manager'), async (req, res) => {
  try {
    const now = new Date();
...
// GET /payroll/items - Admin/Manager: all items, Staff: own items only
router.get("/items", async (req: AuthRequest, res) => {
  try {
    const user = req.user!;
    const allItems = await getPayItems();
    
    // Admin and Manager can see all pay items
    if (user.role === 'admin' || user.role === 'manager') {
      return res.json({ data: allItems });
    }
    
    // Staff can only see their own pay items
    if (user.role === 'staff') {
